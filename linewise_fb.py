...
line_image = line_image[0:2000/(pixel_time*sampling_freq)+img_cols*2]
...
cycs_per_line = img_cols * pixel_time * sampling_freq
buffer_len = eoModulator.dataIn.__len__()
pout_reg = np.zeros(cycs_per_line + buffer_len)
# imaging loop
for i in tqdm(range(0, sample_num, 1)):
    ...
    pout_reg[0:-1] = pout_reg[1:]
    pout_reg[-buffer_len-1:-1] = daConverter.dataOut

    # eoModulator.dataIn[-1] = pout_reg[0]
    # eoModulator.dataIn[0:-1] = eoModulator.dataIn[1:]
    eoModulator.dataIn = pout_reg[0:buffer_len]
